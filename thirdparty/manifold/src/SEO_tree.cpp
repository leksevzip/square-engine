...
struct MeshCompare {
  bool operator()(const std::shared_ptr<SEOLeafNode>& a,
                  const std::shared_ptr<SEOLeafNode>& b) {
...
std::shared_ptr<SEONode> SEONode::Boolean(
    const std::shared_ptr<SEONode>& second, OpType op) {
  if (second->GetNodeType() != SEONodeType::Leaf) {
    // "this" is not a SEOOpNode (which overrides Boolean), but if "second" is
    // and the operation is commutative, we let it built the tree.
    if ((op == OpType::Add || op == OpType::Intersect)) {
      return std::static_pointer_cast<SEOOpNode>(second)->Boolean(
          shared_from_this(), op);
    }
  }
  std::vector<std::shared_ptr<SEONode>> children({shared_from_this(), second});
  return std::make_shared<SEOOpNode>(children, op);
}

std::shared_ptr<SEONode> SEONode::Translate(const vec3& t) const {
  mat3x4 transform = la::identity;
  transform[3] += t;
  return Transform(transform);
}

std::shared_ptr<SEONode> SEONode::Scale(const vec3& v) const {
  mat3x4 transform;
  for (int i : {0, 1, 2}) transform[i][i] = v[i];
  return Transform(transform);
}

std::shared_ptr<SEONode> SEONode::Rotate(double xDegrees, double yDegrees,
...
SEOLeafNode::SEOLeafNode() : pImpl_(std::make_shared<Manifold::Impl>()) {}

SEOLeafNode::SEOLeafNode(std::shared_ptr<const Manifold::Impl> pImpl_)
    : pImpl_(pImpl_) {}
...
std::shared_ptr<const Manifold::Impl> SEOLeafNode::GetImpl() const {
...
std::shared_ptr<SEOLeafNode> SEOLeafNode::ToLeafNode() const {
  return std::make_shared<SEOLeafNode>(*this);
}

std::shared_ptr<SEONode> SEOLeafNode::Transform(const mat3x4& m) const {
  return std::make_shared<SEOLeafNode>(pImpl_, m * Mat4(transform_));
}

SEONodeType SEOLeafNode::GetNodeType() const { return SEONodeType::Leaf; }

std::shared_ptr<SEOLeafNode> ImplToLeaf(Manifold::Impl&& impl) {
  return std::make_shared<SEOLeafNode>(std::make_shared<Manifold::Impl>(impl));
}

std::shared_ptr<SEOLeafNode> SimpleBoolean(const Manifold::Impl& a,
 ...
 */
std::shared_ptr<SEOLeafNode> SEOLeafNode::Compose(
 ...
 */
std::shared_ptr<SEOLeafNode> BatchBoolean(
    OpType operation, std::vector<std::shared_ptr<SEOLeafNode>>& results) {
  ZoneScoped;
  DEBUG_ASSERT(operation != OpType::Subtract, logicErr,
               "BatchBoolean doesn't support Difference.");
  // common cases
  if (results.size() == 0) return std::make_shared<SEOLeafNode>();
  ...
  std::vector<std::shared_ptr<SEOLeafNode>> tmp;
#if MANIFOLD_PAR == 1
 ...
 * possible.
 */
std::shared_ptr<SEOLeafNode> BatchUnion(
  ...
  while (children.size() > 1) {
    ...
    std::vector<std::shared_ptr<SEOLeafNode>> impls;
    for (auto& set : disjointSets) {
      if (set.size() == 1) {
        ...
        impls.push_back(SEOLeafNode::Compose(tmp));
      }
    }
...
SEOOpNode::SEOOpNode(const std::vector<std::shared_ptr<SEONode>>& children,
                     OpType op)
...
SEOOpNode::~SEOOpNode() {
  if (impl_.UseCount() == 1) {
    ...
    std::vector<std::shared_ptr<SEOOpNode>> toProcess;
    auto handleChildren =
        [&toProcess](std::vector<std::shared_ptr<SEONode>>& children) {
          while (!children.empty()) {
            // move out so shrinking the vector will not trigger recursive drop
            auto movedChild = std::move(children.back());
            children.pop_back();
            if (movedChild->GetNodeType() != SEONodeType::Leaf)
              toProcess.push_back(
                  std::static_pointer_cast<SEOOpNode>(std::move(movedChild)));
          }
        };
    handleChildren(*impl);
...
std::shared_ptr<SEONode> SEOOpNode::Boolean(
    const std::shared_ptr<SEONode>& second, OpType op) {
...
std::shared_ptr<SEONode> SEOOpNode::Transform(const mat3x4& m) const {
  auto node = std::make_shared<SEOOpNode>();
  node->impl_ = impl_;
...
struct SEOStackFrame {
  using Nodes = std::vector<std::shared_ptr<SEOLeafNode>>;
  ...
  Nodes* positive_dest;
  Nodes* negative_dest;
  std::shared_ptr<const SEOOpNode> op_node;
  Nodes positive_children;
  Nodes negative_children;

  SEOStackFrame(bool finalize, OpType parent_op, mat3x4 transform,
...
std::shared_ptr<SEOLeafNode> SEOOpNode::ToLeafNode() const {
  ...
  // put the `SEOLeafNode` into. Normally, the `positive_dest` pointer point to
  ...
  // result. This is because `impl` can be shared between `SEOOpNode` that
  // differ in `transform_`, so we want it to be able to share the result.
  // ===========================================================================
  // Recursive version (pseudocode only):
  //
  // void f(SEOOpNode node, OpType parent_op, mat3x4 transform,
  //        Nodes *positive_dest, Nodes *negative_dest) {
  ...
  //                      neg_dest : nullptr;
  //     if (child->GetNodeType() == SEONodeType::Leaf)
  ...
  // }
  while (!stack.empty()) {
    std::shared_ptr<SEOStackFrame> frame = stack.back();
    ...
    if (frame->finalize) {
      switch (frame->op_node->op_) {
        ...
        case OpType::Subtract:
          if (frame->positive_children.empty()) {
            ...
            *impl = {std::make_shared<SEOLeafNode>()};
          } else {
      ...
      frame->op_node->cache_ = std::static_pointer_cast<SEOLeafNode>(
          (*impl)[0]->Transform(frame->op_node->transform_));
      if (frame->positive_dest != nullptr)
        frame->positive_dest->push_back(std::static_pointer_cast<SEOLeafNode>(
            frame->op_node->cache_->Transform(frame->transform)));
      stack.pop_back();
    } else {
      auto add_children =
          [&stack](std::shared_ptr<SEONode>& node, OpType op, mat3x4 transform,
                   SEOStackFrame::Nodes* dest1, SEOStackFrame::Nodes* dest2) {
            if (node->GetNodeType() == SEONodeType::Leaf)
              dest1->push_back(std::static_pointer_cast<SEOLeafNode>(
                  node->Transform(transform)));
            else
              stack.push_back(std::make_shared<SEOStackFrame>(
                  false, op, transform, dest1, dest2,
                  std::static_pointer_cast<const SEOOpNode>(node)));
          };
      // op_node use_count == 2 because it is both inside one SEOOpNode
      // and in our stack.
      ...
      const mat3x4 transform =
          canCollapse ? (frame->transform * Mat4(frame->op_node->transform_))
                      : la::identity;
      SEOStackFrame::Nodes* pos_dest =
          canCollapse ? frame->positive_dest : &frame->positive_children;
      SEOStackFrame::Nodes* neg_dest =
          canCollapse ? frame->negative_dest : &frame->negative_children;
      for (size_t i = 0; i < impl->size(); i++) {
        const bool negative = op == OpType::Subtract && i != 0;
        SEOStackFrame::Nodes* dest1 = negative ? neg_dest : pos_dest;
        SEOStackFrame::Nodes* dest2 =
...
SEONodeType SEOOpNode::GetNodeType() const {
  switch (op_) {
    case OpType::Add:
      return SEONodeType::Union;
    case OpType::Subtract:
      return SEONodeType::Difference;
    case OpType::Intersect:
      return SEONodeType::Intersection;
  }
  // unreachable...
  return SEONodeType::Leaf;
}
